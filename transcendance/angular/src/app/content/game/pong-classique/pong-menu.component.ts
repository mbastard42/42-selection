...
@Component({
...
}) export class PongMenuComponent{
    ...
    constructor(private readonly pongRequest: PongRequest, private readonly authService: AuthService, private readonly userService : UserService, private readonly navService: NavService, private readonly router: Router) {
        ...
        this.navService.setElem({id: 'play_game/game/pong', nextBehavior: this.searchOrJoinGame, down: 'watch_game/game/pong', next: 'cancel_search/game/pong',  prev: 'classic_game/game'})
        this.navService.setElem({id: 'watch_game/game/pong', nextBehavior: this.searchCurrentGamesWatch, up: 'play_game/game/pong', next: 'cancel_search/game/pong', prev: 'classic_game/game'}),
        this.navService.setElem({id: 'cancel_search/game/pong', nextBehavior: this.cancelAction, next: 'play_game/game/pong'})
        
        this.navService.setDefaultElem('play_game', '/game/pong');
    ...
    searchCurrentGamesWatch = async () => {
        ...
        if (this.CurrentGames.length > 0)
          this.navService.setElem({id: 'cancel_search/game/pong', down: 'watch_game_id' + this.CurrentGames[0].id + '/game/pong'})
    }
    ...
    cancelWatchGameSearch() {
        ...
        this.doWatchSearchGame = false;
        this.navService.setElem({id: 'cancel_search/game/pong', nextBehavior: this.cancelAction, next: 'play_game/game/pong', down: 'cancel_search/game/pong'})
    }
  ...
  {
    let lastId = 'cancel_search/game/pong';
    for (let i = 0; i < games.length; i++)
    {
      if (i == 0)
        this.navService.setElem
      this.navService.setElem({
        id: 'watch_game_id' + games[i].id + '/game/pong',
        next: 'game_div/game/watch/' + games[i].id,
        up: lastId,
        down: i == games.length - 1 ? 'watch_game_id' + games[i].id + '/game/pong' : 'watch_game_id' + games[i + 1].id + '/game/pong',
        upBehavior: this.scrollUpGameWatchDiv,
        downBehavior: this.scrollDownGameWatchDiv,
        nextBehavior: () => this.watchGame(games[i].id)
      })
      lastId = 'watch_game_id' + games[i].id + '/game/pong';
    }
  }
  ...
  {
    const response = await this.pongRequest.joinMatchmaking();
    if (response)
    {
  ...
  {
    const response = await this.pongRequest.cancelMatchmaking();
